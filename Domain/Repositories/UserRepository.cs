...
namespace Identity.Infrastructure.Repositories
{
        ...
        where TRoleClaim : IdentityRoleClaim<TKey>
    {
        ...
        public UserRepository(DbContext context, RoleRepository<TKey, IdentityRole<TKey>, TUserRole, TRoleClaim> roleRepository)
        {
            _context = context;
        ...
        {
            return await _context.QueryFirstOrDefaultAsync<TUser>("select IU.* from Users IU,UserLogins IL where IU.Id=IL.UserId and IL.LoginProvider=@loginProvider and IL.providerKey=@providerKey", new { loginProvider = loginProvider, providerKey = providerKey });
        }
        ...
        {
            var result = await _context.QueryAsync("select LoginProvider,ProviderKey from UserLogins  where  UserId=@UserId", new { UserId = id });
        ...
        {
            _context.Execute("insert into Users values (@Id,@UserName,@FullName,@Email,@EmailConfirmed,@PasswordHash,@SecurityStamp,@PhoneNumber,@PhoneNumberConfirmed,@TwoFactorEnabled,@LockoutEndDateUtc,@LockoutEnabled,@AccessFailedCount,@CreatedBy,@CreatedDate,@LastUpdatedBy,@LastUpdatedDate)", user);
        }
        ...
        {
            _context.Execute("insert into UserLogins values (@UserId,@LoginProvider,@ProviderKey)", new { UserId = id, LoginProvider = loginInfo.LoginProvider, ProviderKey = loginInfo.ProviderKey });
        }
